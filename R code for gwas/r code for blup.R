...
qualdat = read.csv("montcalm two year together with average .csv", header=T)
...
## BLUPS
# fit the model
brixmodel = lmer(DII~ (1|LINE) + (1|LOC) + (1|YEAR) + (1|REP%in%LOC:YEAR) + (1|LINE:LOC) + (1|LINE:YEAR))
...
write.csv(brixlineblup, file="DIIBLUPS.csv")
...
lmean = tapply(YIELD, LINE, na.rm=T, mean)
