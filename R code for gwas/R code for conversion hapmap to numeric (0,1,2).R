...
### The following cod is to convert Hapmap SNP data to numerical (0,1,2)
...
GAPIT.HapMap <-
  function(G,SNP.effect="Add",SNP.impute="Middle",heading=TRUE, Create.indicator = FALSE, Major.allele.zero = FALSE){
    ...
    print(dim(GD))
    print ("Succesfuly finished converting HapMap to numeric data !")
...
###Second step, run the following codes to read the hapmap data and convert it to numeric data (-1,0,1) format.

myG <- read.table("widiv_675x496845_filter300.03.hmp.txt", head = FALSE)

GAPIT.HapMap (myG)  
##This is the end of all steps, Please check the output file "Numeric data.csv", remember to delet the labels in the first row and column if you wanna use it in rrBLUP.
